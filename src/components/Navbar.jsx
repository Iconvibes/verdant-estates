...
const Navbar = () => {
  ...
  const headerClass = [
    'sticky top-0 z-50 text-cream transition-all duration-300 ease-out',
    scrolled
    ...
    <header className={headerClass}>
      <nav className={navClass} aria-label="Main navigation">
        ...
        <div className="hidden items-center gap-7 md:flex" role="menubar">
          {links.map((link) => (
            <NavLink key={link.to} to={link.to} className={linkClass} end={link.to === '/'} role="menuitem">
              {link.label}
            </NavLink>
          ))}
          <Link
      ...
      >
        <div className="flex flex-col gap-1 pb-4">
          ...
          <NavLink
            ...
            onClick={() => setOpen(false)}
            role="menuitem"
            ref={(el) => { menuItemsRef.current[links.length] = el }}
            className={({ isActive }) =>
          ...
          <Link
            ...
            onClick={() => setOpen(false)}
            role="menuitem"
            ref={(el) => { menuItemsRef.current[links.length + 1] = el }}
